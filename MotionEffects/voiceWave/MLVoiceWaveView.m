...
//  Copyright © 2016年 ML Inc. All rights reserved.
//

#import "MLVoiceWaveView.h"
#import "MLVolumeQueue.h"

@interface MLVoiceWaveView ()

@property (nonatomic, strong) CADisplayLink *displayLink;
//@property (nonatomic, copy) MLShowLoadingCircleCallback showLoadingCircleCallback;
...
@property (nonatomic, strong) MLVolumeQueue *volumeQueue;
...
@implementation MLVoiceWaveView {
...
{
    @autoreleasepool {
        [[NSThread currentThread] setName:@"com.ml.VoiceWave"];
...
{
    if (self.superview != parentView || !_isStopAnimating) {
...
//    self.backgroundColor = [[UIColor blueColor] colorWithAlphaComponent:0.3];
    ...
    [_secondLine removeFromSuperview];
    [_fillLayerImage removeFromSuperview];
...
- (void)stopVoiceWaveWithShowLoadingViewCallback:(MLShowLoadingCircleCallback)showLoadingCircleCallback
{
    if (_isStopAnimating) {
        return;
    }
...
- (MLVolumeQueue *)volumeQueue
{
    if (!_volumeQueue) {
        self.volumeQueue = [[MLVolumeQueue alloc] init];
    }
