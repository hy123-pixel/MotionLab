...
#import "MLTableRefreshLabelView.h"
#import "MLTableRefreshCurveView.h"

@interface  MLTableRefreshFooterView()
...
@implementation MLTableRefreshFooterView {
    MLTableRefreshLabelView *labelView;
    MLTableRefreshCurveView *curveView;
...
//        [MLGlobalMethod showHUDWithString:@"网络不给力，请稍后重试"];
//        return;
//    }
...
-(void)setUp
{
    ...
    [_refreshButton setTitle:@"上滑加载更多" forState:UIControlStateNormal];
    [_refreshButton setTitleColor:[MLGlobalMethod uiColorFromHexString:@"666666"] forState:UIControlStateNormal];
    ...
    curveView = [[MLTableRefreshCurveView alloc] initWithFrame:CGRectMake(20, 0, 30, self.bounds.size.height)];
    [self insertSubview:curveView atIndex:0];
    curveView.hidden = YES;
    curveView.progress = 1.0;
    
    labelView = [[MLTableRefreshLabelView alloc] initWithFrame:CGRectMake(curveView.frame.origin.x + curveView.frame.size.width + 10, curveView.frame.origin.y, 150, curveView.frame.size.height)];
    labelView.state = UP;
