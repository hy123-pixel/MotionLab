...
@implementation MLWaterWaveView {
    ...
    BOOL increase;      // 增减变化
    MLWaterWaveAnimateType _animateType;//展现时先上升，隐藏时先下降
}
...
{
    _percent = fmax(0.0, fmin(1.0, percent));
    [self resetProperty];
}

-(void)startWave
{
    _animateType = MLWaterWaveAnimateTypeShow;
    [self resetProperty];
...
-(void) stopWave
{
    ...
    _animateType = MLWaterWaveAnimateTypeHide;
}
...
-(void)getCurrentWave:(CADisplayLink *)displayLink
{
    ...
    if (MLWaterWaveAnimateTypeShow == _animateType && currentWavePointY > 2 * waterWaveHeight *(1-_percent)) {
        // 波浪高度未到指定高度 继续上涨
        currentWavePointY -= waveGrowth;
    } else if (MLWaterWaveAnimateTypeHide == _animateType && currentWavePointY < 2 * waterWaveHeight) {
        currentWavePointY += waveGrowth;
    } else if (MLWaterWaveAnimateTypeHide == _animateType && currentWavePointY >= 2 * waterWaveHeight) {
        currentWavePointY = 2 * waterWaveHeight;
        [_waveDisplaylink invalidate];
        _waveDisplaylink = nil;
