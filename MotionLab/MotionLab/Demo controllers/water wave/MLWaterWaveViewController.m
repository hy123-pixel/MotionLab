...
#import "MLWaterWaveViewController.h"
#import "MLWaterWaveView.h"

@interface MLWaterWaveViewController ()

@property (nonatomic, strong) MLWaterWaveView *waterWave;
@property (nonatomic, strong) UIButton *waterWaveShowButton;

@end

@implementation MLWaterWaveViewController
...
- (void)waterWaveShowButtonTouched:(UIButton *)sender
{
    ...
    status++;
    if (status % 2 == 0) {
        [_waterWaveShowButton setTitle:@"显示" forState:UIControlStateNormal];
        [self.waterWave stopWave];
    } else {
        [_waterWaveShowButton setTitle:@"隐藏" forState:UIControlStateNormal];
        [self.view addSubview:self.waterWave];
        [self.waterWave startWave];
    }
}

- (MLWaterWaveView *)waterWave
{
    if (!_waterWave) {
        self.waterWave = [[MLWaterWaveView alloc] init];
